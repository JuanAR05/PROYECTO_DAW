...
session_start();
    ...
    if (!empty($_POST['nombre_usuario']) && !empty($_POST['nombre']) && !empty($_POST['apellido']) && !empty($_POST['correo']) && !empty($_POST['dni']) && !empty($_POST['contrasena']) && !empty($_POST['telefono']) && !empty($_POST['id_localidad'])) {
        ...
        $orden = "INSERT INTO usuario VALUES('" . $user . "','" . $nombre . "','" . $apellido . "','" . $correo . "','" . $dni . "','" . $pass . "'," . $telefono . "," . $localidad . ");";
    if ($conexion->query($orden)) {
        header('Location:http://localhost/PROYECTO_PLAYFUT/soccer-master/index.php');
    } else {
        echo "Error";
    }
} 
    ...
    if (!empty($_POST['nombre_usuario']) && !empty($_POST['contrasena'])) {
        ...
        setcookie('usuario',$_SESSION['usuario'], time()+30000);
        $pass = $_POST['contrasena'];

   
    $select1 = "SELECT CONTRASENA FROM USUARIO WHERE NOMBRE_USUARIO = '" . $_SESSION['usuario'] . "';";
    
    if ($valor = $conexion->query($select1)){
        
        while($columna=$valor->fetch_assoc()){
            if(password_verify($pass, $columna['CONTRASENA']) && $_SESSION['usuario'] == 'admin'){
                echo"Conectado correctamente como administrador<br>";
                header('Location:http://localhost/PROYECTO_PLAYFUT/soccer-master/php/admin.php');
            }else if(password_verify($pass, $columna['CONTRASENA'])){
                echo "Conectado correctamente como Usuario<br>";
                header('Location:http://localhost/PROYECTO_PLAYFUT/soccer-master/index.php');
...
//--------------------------------------FIN SQL--------------------------------------------------------
    ...
    echo "Información no válida o vacía.";
}
    }
?>
    <div class="main">
        ...
        <div class="signup">
            <form method="POST">
                ...
                <input type="number" name="telefono" placeholder="Número teléfono" required="">
        ...
        <div class="login">
            <form method="POST" action="">
                ...
                <br>
                <a href="http://localhost/PROYECTO_PLAYFUT/soccer-master/php/recuperar_pass.php" style="padding-left: 145px;">¿Has olvidado tu contraseña?</a>
            </form>
        </div>
    </div>
